...
const ALLOWED_PATH = new RegExp(
  `^(me|models|conversations(?:/${UUID})?|chat/completions|messages/${UUID}/(?:cancel|retry|feedback|citations/${UUID})|knowledge-bases(?:/${UUID}/documents)?|documents/${UUID}(?:/versions|/upload-complete)?|ingestion-jobs/${UUID}(?:/retry)?|retrieval/search|uploads/${UUID}/content|usage|evaluations/runs(?:/${UUID})?|admin/(?:users(?:/${UUID})?|groups|rag-configs(?:/${UUID}/(?:evaluations|approve|publish|rollback))?|quota-policies/tenant|audit-logs(?:/integrity)?|usage-summary|quality-summary|operations/snapshot|security-incidents(?:/${UUID})?|releases(?:/${UUID}(?:/uat-results|/signoffs|/rollout/(?:start|advance|stop|rollback))?)?))$`,
);
