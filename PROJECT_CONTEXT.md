...
> 状态：S1 工程纵切完成，企业集成条件待关闭  
> 基线版本：s1-v1.0  
> 基线日期：2026-07-15  
> 下一阶段：S2 Model Gateway 与流式聊天（仅合成开发）
...
| 架构与 ADR | ADR-001～013 已接受为开发基线 | `docs/enterprise-qa-system/adr/` |
...
1. 进入 S2 合成开发：Model Gateway、deterministic Fake Provider、REST + SSE、取消/重试、用量/成本与错误归一化。
2. 由用户/业务方回答 S0 开放问题；仅在批准后使用模型沙箱运行基线实验。
3. Platform/IAM 并行补齐企业 OIDC、GitHub dev 自动部署和 Helm/Kubernetes 验证。
4. S2 PR 必须引用 ADR、OpenAPI、本文件约束和 `s1/07-s1-gate-review.md` 的禁止事项。
